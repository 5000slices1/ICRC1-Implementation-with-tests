...
module {
    ...
    func GetAssertParamForTransactionCheck(actualTransaction : ?Transaction, index : ?Nat, description : Text) : ActorSpec.AssertParam<Transaction, Transaction> {
    ...
    func GetAssertParamForTransactionsCheck(actualTransactions : [Transaction], fromTo : ?(fromIndex : Nat, toIndex : Nat), description : Text) : ActorSpec.AssertParam<[Transaction], [Transaction]> {
                ...
                it(
                    ...
                    do {
                        ...
                        let secondResult = assertAllEqualWithDescription([
                            ...
                            GetAssertParamForTransactionsCheck(
                                (await archive.get_transactions({ start = 0; length = 100 })).transactions,
                                ?(0, 100),
                                "0-100",
                            ),

                            GetAssertParamForTransactionsCheck(
                                (await archive.get_transactions({ start = 10; length = 15 })).transactions,
                                ?(10, 25),
                                "10-25",
                            ),

                            GetAssertParamForTransactionsCheck(
                                (await archive.get_transactions({ start = 225; length = 100 })).transactions,
                                ?(225, 325),
                                "225-325",
                            ),

                            GetAssertParamForTransactionsCheck(
                                (await archive.get_transactions({ start = 225; length = 1200 })).transactions,
                                ?(225, 1425),
                                "225-1425",
                            ),

                            GetAssertParamForTransactionsCheck(
                                (await archive.get_transactions({ start = 225; length = 3000 })).transactions,
                                ?(225, 3225),
                                "225-3225",
                            ),

                            GetAssertParamForTransactionsCheck(
                                (await archive.get_transactions({ start = 980; length = 100 })).transactions,
                                ?(980, 1080),
                                "980-1080",
                            ),

                            GetAssertParamForTransactionsCheck(
                                (await archive.get_transactions({ start = 3251; length = 2000 })).transactions,
                                ?(3251, 5251),
                                "3251-5251",
                            ),

                            GetAssertParamForTransactionsCheck(
                                (await archive.get_transactions({ start = 451; length = 3200 })).transactions,
                                ?(451, 3651),
                                "451-3651",
                            ),

                            GetAssertParamForTransactionsCheck(
                                (await archive.get_transactions({ start = 6800; length = 3000 })).transactions,
                                ?(6800, 9000),
                                "6800-9800",
                            ),

                            GetAssertParamForTransactionsCheck(
                                (await archive.get_transactions({ start = 8300; length = 3000 })).transactions,
                                ?(8300, 9000),
                                "8300-11300",
                            ),
